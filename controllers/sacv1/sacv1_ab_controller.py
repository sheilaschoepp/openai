...
from controllers.sacv1.sacv1_agent import SACv1
...
class AbnormalController:
    ...
    def __init__(self):
        ...
        # agent
        self.agent = SACv1(self.env.env_state_dim(),
    ...
    def plot(self):
        ...
        # training: value_loss vs num_updates
        df.plot(x="num_updates", y="value_loss", color="blue", legend=False)
        plt.axvline(x=(self.parameters["n_time_steps"] - self.parameters["batch_size"]) * self.parameters["model_updates_per_step"], ymin=0, ymax=1, color="red", linewidth=2, alpha=0.3)  # malfunction marker
        plt.xlabel("updates")
        plt.ylabel("loss", rotation="horizontal", labelpad=30)
        plt.title("Value Loss")
        pss.plot_settings()
        plt.savefig(jpg_foldername + "/value_loss.jpg")
        plt.close()
    ...
    def save_data(self):
        ...
        loss_data_df = pd.DataFrame({"num_updates": self.loss_data[:, 0],
                                     "value_loss": self.loss_data[:, 1],
                                     "q_value_loss_1": self.loss_data[:, 2],
                                     "q_value_loss_2": self.loss_data[:, 3],
                                     "policy_loss": self.loss_data[:, 4]})
