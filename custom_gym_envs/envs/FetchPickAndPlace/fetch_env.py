...
from custom_gym_envs.envs.FetchPickAndPlace import rotations, robot_env, utils
...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        elif norm_grip_object < 0.015:
            reward = np.exp(np.exp(-norm_grip_object))
        else:
            reward = - norm_grip_object

        # if norm_grip_object < 0.005:
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate([
            grip_pos, object_pos.ravel(), object_rel_pos.ravel(), gripper_state, object_rot.ravel(),
            object_velp.ravel(), object_velr.ravel(), grip_velp, gripper_vel, self.goal.copy()
        ])
