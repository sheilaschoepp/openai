...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        is_success = self._is_success(achieved_goal, goal)
        ...
        # Extract joint positions, substituting 1.5 radians for the faulty sensor

        shoulder_pan = qpos['robot0:shoulder_pan_joint'] 

        shoulder_lift = 1.5  

        upperarm_roll = qpos['robot0:upperarm_roll_joint'] 

        elbow_flex = qpos['robot0:elbow_flex_joint'] 

        forearm_roll = qpos['robot0:forearm_roll_joint'] 

        wrist_flex = qpos['robot0:wrist_flex_joint'] 

        wrist_roll = qpos['robot0:wrist_roll_joint'] 


        # Weighted sum of joint positions, excluding the faulty joint

        joint_positions_sum = (0.2 * shoulder_pan + 0.0 * shoulder_lift + 0.2 * upperarm_roll + 

                               0.2 * elbow_flex + 0.2 * forearm_roll + 0.2 * wrist_flex + 0.2 * wrist_roll)
    

        # Performance measure encouraging all non-faulty joints into action for fault compensation

        performance_measure = 1.0 / (1 + np.abs(joint_positions_sum))
    

        # Reward combining distance and performance measure

        reward = -distance + performance_measure
