...
from custom_gym_envs.envs.fetchreach.FetchReachEnv_v4_BrokenShoulderLiftSensor_E_v12 import robot_env, rotations, utils  # modification here
...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        # Extract joint positions and velocities while ignoring the faulty sensor value

        qpos_dict = info['qpos_dict'].copy()

        qvel_dict = info['qvel_dict'].copy()
    

        # Remove faulty sensor data entry

        qpos_dict.pop('robot0:shoulder_lift_joint', None)

        qvel_dict.pop('robot0:shoulder_lift_joint', None)
    

        # Incentivize exploration of healthy joints to offset the faulty sensor

        joint_positions_penalty = sum(np.abs(value) for value in qpos_dict.values())

        joint_velocities_penalty = sum(np.abs(value) for value in qvel_dict.values())

        penalty = 0.01 * (joint_positions_penalty + joint_velocities_penalty)
    

        # Main reward is negative distance
