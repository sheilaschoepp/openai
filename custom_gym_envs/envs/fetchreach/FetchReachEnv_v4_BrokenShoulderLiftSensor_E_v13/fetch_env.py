...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        shoulder_lift_joint_torque = info['actuator_torques']['robot0:shoulder_lift_joint'] # Torque from shoulder lift joint actuator

        shoulder_lift_joint_constraint_force = info['constraint_forces']['robot0:shoulder_lift_joint'] # Constraint force from shoulder lift joint
    

        # Reward Components

        # Distance between achieved position and target position

        distance = np.linalg.norm(achieved_position - target_position)
    

        # Penalize excessive torque from shoulder lift joint

        torque_penalty = np.clip(shoulder_lift_joint_torque - 0.5, 0, 1.0) # Penalize high torques
    

        # Reward Based on Torque and Constraint Forces
