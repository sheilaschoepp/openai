...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        torque_penalty_coefficient = 0.1

        stability_coefficient = 0.05

        failure_penalty = 10.0

        shoulder_lift_joint_value = 1.5


        # Actuator torques

        actuator_torques = info['actuator_torques']
    

        # Specific torque values

        torque_shoulder_lift = actuator_torques['robot0:shoulder_lift_joint']
    

        # Penalty for high torques to avoid erratic movements

        torque_penalty = sum(abs(torque) for torque in actuator_torques.values()) * torque_penalty_coefficient
    

        # Check if the shoulder lift joint is in the faulty position

        qpos_shoulder_lift = shoulder_lift_joint_value

        if qpos_shoulder_lift == 1.5:

            stability_penalty = abs(torque_shoulder_lift) * stability_coefficient
        ...
        reward = distance_reward - torque_penalty - stability_penalty
    

        return reward if d < self.distance_threshold else reward - failure_penalty, -d
