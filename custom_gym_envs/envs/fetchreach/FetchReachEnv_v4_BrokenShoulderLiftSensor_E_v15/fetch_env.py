...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):  
        ...
        joint_positions = info['qpos_dict']  

        joint_velocities = info['qvel_dict']  
        ...
        smoothness_penalty = np.sum([np.abs(joint_velocities[j]) for j in important_joints])  
        ...
        for j in important_joints:  
            ...
            displacement_factor += np.linalg.norm(joint_positions[j] - goal)  

        displacement_factor /= len(important_joints)  
      
        return -d - 0.1 * smoothness_penalty - 0.05 * displacement_factor, -d
