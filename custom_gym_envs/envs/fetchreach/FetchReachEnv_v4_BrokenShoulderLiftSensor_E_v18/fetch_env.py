...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        # Add a large bonus for reaching the goal within a threshold
        ...
        # Penalize torque applied at the faulty joint to encourage reliance on other joints

        torque_shoulder_lift = np.abs(info['actuator_torques'].get('robot0:shoulder_lift_joint', 0.0))

        reward -= 0.1 * torque_shoulder_lift


        # Penalize total actuator torques to promote efficient movements

        total_torque = np.sum([np.abs(torque) for joint, torque in info['actuator_torques'].items()])

        reward -= 0.01 * total_torque
