...
def plot_complete_adaptation(directory):
    """
    Plot complete adaptation for all four settings of a single fault.
    ...
    x_fault_onset = ordered_settings[0][4].iloc[eval_fault_onset, 0] / x_divisor

    # plot normal performance

    x = ordered_settings[0][4].iloc[:eval_fault_onset, 0] / x_divisor
    y = ordered_settings[0][4].iloc[:eval_fault_onset, 1]

    # 95 % confidence interval
    lb = y - CI_Z * ordered_settings[0][5].iloc[:eval_fault_onset, 1]
    ub = y + CI_Z * ordered_settings[0][5].iloc[:eval_fault_onset, 1]

    plt.plot(x, y, color=palette_colours[0], label="pre-fault")
    plt.fill_between(x, lb, ub, color=palette_colours[0], alpha=0.2)

    # plot fault performance

    for i in range(4):

        x = ordered_settings[i][4].iloc[eval_fault_onset - 1:, 0] / x_divisor
        y = ordered_settings[i][4].iloc[eval_fault_onset - 1:, 1]

        # 95 % confidence interval
        lb = y - CI_Z * ordered_settings[i][5].iloc[eval_fault_onset - 1:, 1]
        ub = y + CI_Z * ordered_settings[i][5].iloc[eval_fault_onset - 1:, 1]
...
if __name__ == "__main__":
    ...
    plot_complete_adaptation(os.path.join(ppo_data_dir, "v1"))

    # v2

    plot_complete_adaptation(os.path.join(ppo_data_dir, "v2"))

    # v3

    plot_complete_adaptation(os.path.join(ppo_data_dir, "v3"))

    # v4

    plot_complete_adaptation(os.path.join(ppo_data_dir, "v4"))
    ...
    plot_complete_adaptation(os.path.join(sac_data_dir, "v1"))

    # v2

    plot_complete_adaptation(os.path.join(sac_data_dir, "v2"))

    # v3

    plot_complete_adaptation(os.path.join(sac_data_dir, "v3"))

    # v4

    plot_complete_adaptation(os.path.join(sac_data_dir, "v4"))
    ...
    plot_complete_adaptation(os.path.join(ppo_data_dir, "v6"))
    ...
    plot_complete_adaptation(os.path.join(sac_data_dir, "v6"))
