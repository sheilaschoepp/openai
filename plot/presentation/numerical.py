...
def percent_recovery(d):
    ...
    for dir_ in os.listdir(d):
        ...
        min = prefault.iloc[0, :].mean()

        prefault_interval = prefault.iloc[-interval_size:, :]
        prefault_interval = prefault_interval.stack().values.mean()

        max = prefault_interval
        ...
        while not finished:
            ...
            postfault_interval = postfault_interval.stack().values.mean()
