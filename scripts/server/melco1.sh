...
#for s in {0..4}
#do
#  tmux new-session -d -s sacdefault$s "CUDA_VISIBLE_DEVICES=0 python $SAC_N_CONTROLLER_ABSOLUTE_PATH -a -c --resumable -s $s"
#done
#
#for s in {5..9}
#do
#  tmux new-session -d -s sacdefault$s "CUDA_VISIBLE_DEVICES=1 python $SAC_N_CONTROLLER_ABSOLUTE_PATH -a -c --resumable -s $s"
#done
