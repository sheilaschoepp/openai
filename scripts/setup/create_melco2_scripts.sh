...
for s in {5..9}
do
  echo 'tmux new-session -d -s sac'$s' "CUDA_VISIBLE_DEVICES=1 python $SAC_AB_CONTROLLER_ABSOLUTE_PATH -e FetchReachEnv-v1 -t 20000000 -c -f $RESUME_FILE/seed'$s'"' >> melco2.sh
done
